...
function setup() {
  ...
  const data = {
    eventus_events: [{ code: 'VV-TEST', name: 'Test event', phone: '5512345678', date: '2027-01-01', type: 'xv', photo_url: 'storage://eventus-photos/VV-TEST/cover.jpg', gallery: ['storage://eventus-photos/VV-TEST/gallery.jpg'] }, { code: 'VV-OTHER', name: 'Private other event' }],
  ...
  global.fetch = async (address, options = {}) => {
    ...
    const matching = row => ['id', 'code', 'event_code', 'phone'].every(key => !url.searchParams.has(key) || String(row[key]) === url.searchParams.get(key).slice(3));
...
test('registration ignores privilege fields, generates identifiers and issues usable event token', async () => {
  const { post, state } = setup();
  const result = await post({ op: 'register', body: { name: 'New event', phone: '5599998888', type: 'xv', date: '2027-01-01', code: 'VV-OTHER', activated: true, total_cost: 9000 } });
...
test('guest invitation tokens authorize only their own RSVP and never event management', async () => {
  ...
  assert.match(body.event.photo_url, /\/object\/sign\/eventus-photos\/VV-TEST\/cover.jpg\?token=/);
  assert.match(body.event.gallery[0], /\/object\/sign\/eventus-photos\/VV-TEST\/gallery.jpg\?token=/);
...
test('HTML rendering escapes stored text, JS arguments and unsafe URL schemes', () => {
  ...
  assert.ok(source.includes('issueEventLink(code)'));
  assert.ok(source.includes('op: "guest-token"'));
  assert.ok(source.includes('escapeHtml(brief.message)'));
  assert.ok(source.includes('escapeHtml(brief.venue)'));
  assert.ok(source.includes('escapeHtml(brief.notes)'));
  assert.ok(source.includes('escapeHtml(brief.dress_code)'));
  assert.ok(source.includes('escapeHtml(brief.event_time)'));
  assert.ok(source.includes('parts.push(escapeHtml(brief.song))'));
  assert.ok(source.includes('var url = safeUrl(p)'));
  assert.ok(source.includes('new URLSearchParams(location.hash.slice(1)).get("access")'));
  assert.ok(source.includes('sessionStorage.setItem("eventus_access",token)'));
  assert.ok(source.includes('sessionStorage.getItem("eventus_access")'));
  assert.ok(source.includes('sessionStorage.removeItem("eventus_access")'));
  assert.ok(source.includes('escapeHtml(E.invitation_comments).replace(/\\n/g, "<br>")'));
  assert.ok(source.includes('const link = await issueEventLink(code)'));
  assert.ok(!source.includes('brief.venue +'));
  assert.ok(!source.includes('brief.notes +'));
  assert.equal((source.match(/var url = safeUrl\(p\)/g) || []).length, 3);
  assert.ok(!source.includes('/storage/v1/object/eventus-photos/'));
