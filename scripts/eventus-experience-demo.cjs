...
function bootstrap() {
  window.addEventListener('load', () => {
    ...
    guests.push(...[{id:10,name:'Rosa Hernández',companions:3,mesa:5,status:'confirmed'},{id:11,name:'Jorge Hernández',companions:2,mesa:5,status:'confirmed'},{id:12,name:'Lucía Méndez',companions:1,mesa:5,status:'confirmed'},{id:13,name:'Fernanda Ortiz',companions:4,mesa:6,status:'confirmed'},{id:14,name:'Ricardo Salas',companions:4,mesa:6,status:'confirmed'},{id:15,name:'Patricia Gómez',companions:1,mesa:7,status:'confirmed'},{id:16,name:'Miguel Ángel Ruiz',companions:3,mesa:7,status:'confirmed'},{id:17,name:'Daniela Flores',companions:2,mesa:8,status:'confirmed'},{id:18,name:'Óscar Medina',companions:1,mesa:8,status:'confirmed'},{id:19,name:'Paola Cruz',companions:0,mesa:8,status:'confirmed'},{id:20,name:'Samuel Ríos',companions:3,mesa:8,status:'confirmed'},{id:21,name:'Gabriela Soto',companions:5,mesa:9,status:'confirmed'},{id:22,name:'Alejandro Vargas',companions:1,mesa:10,status:'confirmed'},{id:23,name:'Carmen Aguilar',companions:2,mesa:10,status:'confirmed'},{id:24,name:'Isabel Navarro',companions:3,mesa:11,status:'confirmed'},{id:25,name:'Héctor Campos',companions:3,mesa:11,status:'confirmed'},{id:26,name:'Mariana Saba',companions:0,mesa:12,status:'confirmed'},{id:27,name:'Tomás Guerrero',companions:1,mesa:12,status:'confirmed'},{id:28,name:'Natalia Rojas',companions:2,mesa:12,status:'confirmed'},{id:29,name:'Jimena Castillo',companions:4,mesa:13,status:'confirmed'},{id:30,name:'Arturo León',companions:4,mesa:13,status:'confirmed'},{id:31,name:'Verónica Peña',companions:1,mesa:14,status:'confirmed'},{id:32,name:'Eduardo Silva',companions:2,mesa:16,status:'confirmed'},{id:33,name:'Adriana Mora',companions:1,mesa:16,status:'confirmed'},{id:34,name:'Claudia Reyes',companions:2,mesa:null,status:'confirmed'},{id:35,name:'Raúl Domínguez',companions:0,mesa:null,status:'confirmed'},{id:36,name:'Andrea Paz',companions:3,mesa:null,status:'pending'}].map(g=>({...g,version:1})));
    const tables = Array.from({length:16}, (_,i)=>({mesa:i+1,capacity:10,x:0,y:0,version:1}));
    ...
    const storageKey='eventus-salon-plan-demo-v2';
    ...
    normalize();
    apiCall = async b => {
      if(b.op==='tables-get'){for(let n=tables.length+1;n<=E.numMesas;n++)tables.push({mesa:n,capacity:10,x:0,y:0,version:1});persist();}
    ...
    E={code:'DEMO-LOCAL',name:'Evento de demostración',type:'xv',date:'2027-05-01',activated:true,numMesas:16,hours:6,guests:structuredClone(guests),program:structuredClone(program),payments:[],checklist:[],menuSel:{}};
    show();goPanel('mesas');
