...
const COLUMNS: Record<string, string[]> = {
  eventus_events: "code,name,type,date,hours,total_cost,contract_date,num_mesas,menu_sel,phone,activated,invitation_enabled,invitation_status,invitation_brief,invitation_url,invitation_comments,dress_code,event_time,created_at".split(","),
...
const CLIENT_WRITE: Record<string, string[]> = {
  eventus_events: "num_mesas,menu_sel,invitation_brief,invitation_status,invitation_comments,dress_code,event_time".split(","),
...
async function privateImages(value: any, code: string, cache = new Map<string, Promise<string>>()): Promise<any> {
  ...
  if (typeof value !== "string" || !value.startsWith(SUPABASE_URL + "/storage/v1/object/")) return value;
  const prefix = SUPABASE_URL + "/storage/v1/object/";
  const path = value.slice(prefix.length).split("?")[0].replace(/^(public|sign)\//, "");
  ...
  if ("invitation_brief" in result && result.invitation_brief !== null) {
    ...
    for (const [key, value] of Object.entries(brief)) {
      if (["photos", "reference_photos", "solo_photos", "carousel_photos"].includes(key)) {
        if (!Array.isArray(value) || value.length > 7 || value.some(url => typeof url !== "string" || url.length > 3000 || !/^https:\/\//.test(url))) fail("invalid brief photos", 400);
...
export async function GET(req: Request) {
  return protectedHandler(async () => {
    ...
    const events = await sb("GET", "eventus_events", new URLSearchParams({ code: "eq." + access.code, select: "code,name,type,date,event_time,dress_code,invitation_url,invitation_status" }));
...
export async function POST(req: Request) {
  return protectedHandler(async () => {
    ...
    if (b.op === "register") {
      ...
      const row = { code, name, phone, date: body.date, type: body.type, hours: 7, total_cost: 0, contract_date: new Date().toISOString().slice(0, 10), num_mesas: 20, menu_sel: {}, activated: false, invitation_enabled: false, invitation_status: "sin_brief" };
...
async function upload(req: Request) {
  ...
  const url = SUPABASE_URL + "/storage/v1/object/public/eventus-photos/" + path;
  return j({ url: await privateImages(url, code) });
}
